...
void solve() {
    string s;
    cin >> s;
    int countZero = 0, countOne = 0, start = 0;
    fo(i, 0, s.size()) {
        if (s[i] == '0') countZero++;
        else countOne++;
        if (countOne == countZero) {
            fo(j, start, start + countOne + countZero) cout << s[j];
            cout << nline;
            countZero = countOne = 0;
            start = i + 1;
        }
    }
    if (countOne == s.size() or countZero == s.size()) cout << -1 << nline;
    if (countOne != countZero)
        fo(i, s.size() - 1 - min(countOne, countZero), s.size()) {
        cout << s[i];
    }
    cout << nline << nline;
}
