...
void solve() {
    ...
    vmmn(dp, n, n);
    dp[0][0] = 1;
    int ans = INT_MIN, posI, posJ;
    fo(i, 0, n) dp[i][i] = 1;
    fo(i, 1, n) dp[i - 1][i] = (s[i] == s[i - 1]) ? 1 : 0;
    fo(j, 0, n) {
        fo(i, 0, n - 1) {
            if (!dp[i][j])
                dp[i][j] = ((s[i] == s[j]) and dp[i + 1][j - 1] ? 1 : 0);
            if (dp[i][j])
                if (ans < j - i + 1) posI = i, posJ = j, ans = j - i + 1;
        }
    }
    string answer = "";
    fo(i, posI, posJ + 1) answer += s[i];
    cout << answer;
}
